...
var Session = class {
  constructor(sessionID, studentId, expiration) {
...
// source/auth/credentials.ts
var ClientCredentials = class {
  constructor(user, password, isPasswordHashed = false) {
    this.user = user;
...
async function createSession(credentials, fetch) {
  const url = "https://leoalumnos-svc.udg.mx/alum/api/login/validar";
  const res = await fetch.fetch(url, {
    method: "POST",
    body: JSON.stringify({
      usr: credentials.user,
      pwd: credentials.isPasswordHashed ? credentials.password : await hashPassword(credentials.password)
    })
  });
...
export { ClientCredentials, HttpError, Session, auth_default, buildURL, createISS_default, createSession };
